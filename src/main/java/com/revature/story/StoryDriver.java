...
public class StoryDriver {
	...
	**/
	...
	//model => where POJO's are characters
	//exceptions
	// Driver => story teller
	
	//pencil box  => home; array of writing utensils?
	// interface => write()
	// abstract class => writing utensils
		//double tip size
		//String color
		//boolean hasClip
		
		
	// 	child: ink(ball point, sharpie) or graphite(pencil, mechanical pencils) 
	// 		pens, pencils, sharpies, markers
	
	
	/* Pens, sharpies,and markers already come with a tip but pencil needs to work towards that
	 * 
	 * 
	 * pencils are scared of the sharpener but it turns out that the sharpener 
	 * can help polish the pencil's dull points. Pencil gets bullied? but improves itself! 
	 */
	
	// PencilBoxDoesNotCloseException => checked exception
	// RanOutOfLeadException => runtime exception
	...
	public static void main(String[] args) {
		...
		System.out.println("Due to its popularity, it felt like it had the right to bully the " + pencil.getName() + ".");
		
		
		try {
			fountainPen.bully(pencil);
		} catch (NoBullyingException nbe) {
			System.out.println("No bullying allowed.");
		}
		...
		try {
			...
			System.out.println("Fatal error. Pencil not detected.");
		}
		...
		try {
			...
			System.out.println(" \"Dang it, not again...\" ");
		}
		
		System.out.println(" \"While you were busy basking in your ego, I took the time to improve myself. Now I hava a " + pencil.getTipSize() + 
				" tip. Just because I started out with a thicker tip doesn't mean I can't get better. You might start out with a good tip size but "
				+ "that's going to remain static while I improved.\"");
	}	
}
